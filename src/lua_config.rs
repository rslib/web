//! Lua configuration loader for rs-web
//!
//! This module provides Lua-based configuration, replacing TOML.
...
use crate::config::{
    BuildConfig, Config, EncryptionConfig, GraphConfig, HighlightConfig, ImagesConfig, PathsConfig,
    PermalinksConfig, RssConfig, SectionConfig, SectionsConfig, SeoConfig, SiteConfig,
    TemplatesConfig, TextConfig,
};
...
/// Stored Lua state for computed values and filters
pub struct LuaConfig {
    lua: Lua,
    /// Computed functions to be called after sections are loaded
    pub computed: HashMap<String, mlua::RegistryKey>,
    /// Custom filter functions for Tera templates
    pub filters: HashMap<String, mlua::RegistryKey>,
    /// Custom template functions
    pub functions: HashMap<String, mlua::RegistryKey>,
    /// Computed pages function (generates dynamic pages)
    pub computed_pages: Option<mlua::RegistryKey>,
    /// Before build hook
    pub before_build: Option<mlua::RegistryKey>,
    /// After build hook
    pub after_build: Option<mlua::RegistryKey>,
}

impl LuaConfig {
    /// Load config from a Lua file
    pub fn load<P: AsRef<Path>>(path: P) -> Result<(Config, Self)> {
        let lua = Lua::new();
        ...
        let config_table: Table = lua
            ...
            .eval()
            .map_err(|e| {
                anyhow::anyhow!("Failed to execute config file {:?}: {}", path.as_ref(), e)
            })?;
        ...
        // Parse the config table into Config struct
        let config = parse_config(&config_table)
    ...
    pub fn call_computed(&self, name: &str, sections_json: &str) -> Result<serde_json::Value> {
        ...
        self.call_computed_inner(name, key, sections_json)
            .map_err(|e| anyhow::anyhow!("Failed to call computed '{}': {}", name, e))
    }

    fn call_computed_inner(
        &self,
        _name: &str,
        ...
        // Parse JSON string to serde_json::Value, then convert to Lua
        let json_value: serde_json::Value = serde_json::from_str(sections_json)
            .map_err(|e| mlua::Error::RuntimeError(format!("Invalid JSON: {}", e)))?;
        let sections: Value = self.lua.to_value(&json_value)?;

        let result: Value = func.call(sections)?;
        let json_value: serde_json::Value = self.lua.from_value(result)?;
    ...
    pub fn call_computed_pages(&self, sections_json: &str) -> Result<Vec<ComputedPage>> {
        ...
        self.call_computed_pages_inner(key, sections_json)
            .map_err(|e| anyhow::anyhow!("Failed to call computed_pages: {}", e))
    }

    fn call_computed_pages_inner(
        &self,
        key: &mlua::RegistryKey,
        sections_json: &str,
    ) -> mlua::Result<Vec<ComputedPage>> {
        let func: Function = self.lua.registry_value(key)?;
        ...
        let result: Value = func.call(sections)?;
        let pages: Vec<ComputedPage> = self.lua.from_value(result)?;
...
/// Parse the config table into the Config struct
fn parse_config(table: &Table) -> mlua::Result<Config> {
    ...
    let sections = parse_sections_config(table)?;
...
fn parse_sections_config(table: &Table) -> mlua::Result<SectionsConfig> {
    let mut sections = HashMap::new();
...
mod tests {
    ...
    fn test_minimal_lua_config() {
        ...
        let config = parse_config(&table).unwrap();
