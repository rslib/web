...
fn process_headings(html: &str) -> String {
    ...
    // H1: Title with double underline
    ...
    // H2: Section with single underline
...
/// Format a single table as simple aligned text
...
/// Process lists with proper indentation and markers
fn process_lists(html: &str) -> String {
    let mut text = html.to_string();

    // Ordered lists - replace with numbered items
    ...
    text = ol_re
        .replace_all(&text, |caps: &regex::Captures| {
            ...
            let mut result = String::new();
            result.push('\n');
            for (i, li_cap) in li_re.captures_iter(content).enumerate() {
                let item = strip_tags(&li_cap[1]).trim().to_string();
                // Wrap long lines with proper indentation
                let wrapped = wrap_text(&item, 68);
    ...
    // Unordered lists - replace with bullet points
    ...
    text = ul_re
        .replace_all(&text, |caps: &regex::Captures| {
            ...
            let mut result = String::new();
            result.push('\n');
            for li_cap in li_re.captures_iter(content) {
                let item = strip_tags(&li_cap[1]).trim().to_string();
                let wrapped = wrap_text(&item, 68);
                for (j, line) in wrapped.lines().enumerate() {
                    if j == 0 {
...
/// Helper to create a boxed line with proper padding
...
/// Helper to create a centered boxed line
